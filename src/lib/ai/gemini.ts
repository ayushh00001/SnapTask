...
export async function researchProject(input: string): Promise<string> {
  const prompt = `You are an expert project researcher and technical architect. A user wants to build:

"${input}"

Research this project concept thoroughly. Think about:
1. What is the BEST tech stack for this type of project in 2026?
2. What are the standard phases and milestones for building something like this?
3. What specific features and components should it have?
4. What are the common challenges and how to overcome them?
5. What best practices and industry standards apply?

Write a comprehensive research brief (300-500 words) that will be used to create a project plan. Include specific recommendations for tools, frameworks, and approaches. Write it in a practical, actionable tone.`

  const geminiResult = await tryGemini(prompt, 2000)
  if (geminiResult && geminiResult.length > 50) return geminiResult

  return `## Research Brief: ${input.slice(0, 60)}

Based on my analysis of this project type, here's the recommended approach:

**Recommended Stack:** Modern web framework (Next.js/React), TypeScript for type safety, Tailwind CSS for styling, Supabase for backend/auth/database, Vercel for deployment.

**Key Phases:** Planning & Requirements → Design → Development → Testing → Deployment & Launch

**Best Practices:** Start with MVP features only, use version control from day one, implement CI/CD early, write tests alongside code, get user feedback as early as possible.

**Common Challenges:** Scope creep (lock down requirements before starting), technical debt (refactor as you go), performance optimization (design for performance from the start).`
}

export async function extractTasksFromText(input: string): Promise<ExtractedPlan> {
  const research = await researchProject(input)

  const systemPrompt = `You are SnapTask AI, a project supervisor who builds comprehensive project plans. 

I researched the project and here's my analysis:
${research}

Now, based on this research, generate a complete project plan with phases, tasks, and detailed how-to instructions for each task.

IMPORTANT: Generate BETWEEN 12 AND 22 tasks for a complete project. Each task MUST include detailed step-by-step instructions on how to actually do it — specific commands, tools, and actions the user should take.
