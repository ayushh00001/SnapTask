...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoadingMethod('submit')
    const supabase = createClient()
    const { error } = await supabase.auth.signInWithPassword({ email, password })
    if (error) { toast.error(error.message); setLoadingMethod(null); return }
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    ...
    if (error) { toast.error(error.message); setLoadingMethod(null) }
  }
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="max-w-sm w-full animate-slide-up">
          ...
          <div className="mt-8 space-y-3">
            <Button variant="secondary" className="w-full flex items-center justify-center gap-2.5 h-11" onClick={() => handleOAuth('google')} loading={loadingMethod === 'google'}>
            ...
            <Button variant="secondary" className="w-full flex items-center justify-center gap-2.5 h-11" onClick={() => handleOAuth('github')} loading={loadingMethod === 'github'}>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center"><span className="px-3 text-xs text-text-muted bg-bg">or continue with email</span></div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <Button type="submit" loading={loadingMethod === 'submit'} className="w-full">Sign in</Button>
          </form>
          <Button variant="secondary" className="w-full mt-3" onClick={handleMagicLink} loading={loadingMethod === 'magiclink'}>
            <svg className="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
            Send magic link
          </Button>
          <p className="mt-8 text-center text-sm text-text-secondary">
            No account? <Link href="/signup" className="text-accent font-semibold hover:text-accent-hover">Sign up</Link>
          </p>
        </div>
      </div>
        ...
        <div className="max-w-md relative">
          <div className="w-16 h-16 rounded-2xl bg-white/10 backdrop-blur-sm flex items-center justify-center mb-8">
            <svg className="w-8 h-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813 2.846a4.5 4.5 0 01-3.09 3.09L2.25 22.5l.813-2.846a4.5 4.5 0 013.09-3.09L9 15.904zM18 8.625l-2.813 1.25a2.25 2.25 0 01-1.374 0L10.5 9.75M20.25 6l-2.25-1.125a2.25 2.25 0 00-1.5 0L14.25 6l2.25 1.125a2.25 2.25 0 001.5 0L20.25 6z" />
            </svg>
          </div>
          ...
          <div className="mt-8 flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-white/15 backdrop-blur-sm flex items-center justify-center text-white font-bold text-sm">SC</div>
            <div>
