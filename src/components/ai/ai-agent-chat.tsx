    ...
    <div className="flex flex-col h-[500px]">
      <div className="flex-1 overflow-y-auto px-1 space-y-3">
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-gray-100 text-gray-500 rounded-2xl rounded-bl-md px-4 py-2.5 text-sm border border-gray-200">
      ...
      <div className="flex items-end gap-2 mt-3 pt-3 border-t border-gray-200">
        <input
          ...
          className="flex-1 bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/20 transition-all"
          disabled={loading}
        />
...
function formatMessage(text: string): React.ReactNode {
  if (text.startsWith('**') && text.endsWith('**')) {
    return <strong className="text-gray-900">{text.slice(2, -2)}</strong>
  }
